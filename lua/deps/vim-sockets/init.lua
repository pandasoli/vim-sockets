...
local Logger = require 'vim-sockets.lib.log'
...
function VimSockets:setup(dep_path, vim_events)
...
function VimSockets:emit(event, data, callback)
  Logger:log('emit', 'Emitting event', event, 'to', #self.sockets, 'sockets')
...
function VimSockets:emit_to(socket, event, data, callback)
  ...
  self:call_remote_method(socket, 'receive_data', { props }, function(err)
    if err then
      Logger:log('emit_to', 'Error emitting to', socket .. ':', err)
    end
...
function VimSockets:receive_data(props)
  Logger:log('receive_data', 'Receiving event', props.event, 'from', props.emitter)
